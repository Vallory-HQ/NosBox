...
NosYamlPath = confDir + "/NosBox.yaml"
require File.expand_path(File.dirname(__FILE__) + '/scripts/NosBox.rb')
...
Vagrant.configure("2") do |config|
  ...
  if File.exist? NosYamlPath then
    settings = YAML::load(File.read(NosYamlPath))
  else
    abort "NosBox settings file not found in #{confDir}"
  end

  NosBox.configure(config, settings)
  ...
  # setup 
  # fix UTF-8 bug in vallory/nosbox
